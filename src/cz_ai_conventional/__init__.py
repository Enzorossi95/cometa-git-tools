...
# Ejecutar setup al importar el módulo
setup_global_config()
...
class AIConventionalCz(BaseCommitizen):
    ...
    def _setup_gemini(self):
        ...
        try:
            ...
            self.model = genai.GenerativeModel(model_name='gemini-pro', generation_config=generation_config)
    ...
    def _get_git_changes(self, with_print=False) -> str:
        try:
            ...
            staged_diff = subprocess.check_output(
                ['git', 'diff', '--cached'], stderr=subprocess.STDOUT, universal_newlines=True
            )

            if not staged_diff:
                if with_print:
                    print('[yellow]No staged changes found, checking unstaged changes...[/yellow]')
                staged_diff = subprocess.check_output(
    ...
    def _generate_ai_commit_message(self, with_print=False) -> Dict[str, str]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if with_print:
                    print(f'[red]Error generating commit message (attempt {attempt + 1}): {e}[/red]')
                if attempt < max_retries - 1:
                    ...
                    if with_print:
                        print('[red]Failed to generate commit message after all retries[/red]')
                    return None
    ...
    def _generate_full_message(self, with_print=False):
        ...
        if with_print:
            ...
            print(f'[blue]│[/blue] [bold green]{first_line}[/bold green] [blue]│[/blue]')
            print('[blue]╰' + '─' * (len(first_line) + 4) + '╯[/blue]')
